    ...
    public struct DynamicSequence : IDisposable
    {
        ...
        public void WriteMove(int length, out Span<byte> writePosition)
        {
            ...
            if (m_Buffers.Count == 0)
            {
                ...
                var arrayWrapper = Envirment.Current.GetModule<PoolModule>().Get<ArrayWrapper<byte>, ArrayWrapper<byte>.Poolable>();
                arrayWrapper.Preserve(bufferSize);
                m_Buffers.Add(new ArrayBuffer<byte>(arrayWrapper, 0));
                WritePosition = 0;
                WriteIndex = 0;
            }
            ...
            if (WritePosition + length > m_Buffers[WriteIndex].Buffer.Length)
            {
                ...
                {
                    Envirment.Current.GetModule<PoolModule>().Set<ArrayWrapper<byte>, ArrayWrapper<byte>.Poolable>(m_Buffers[lastIndex].Buffer);
